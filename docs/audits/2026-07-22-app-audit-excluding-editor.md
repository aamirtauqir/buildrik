...
**Week 2 — authorization: ✅ DONE 2026-07-22** — C1 applyToSite→ADMIN (+ cloneFromSite/use→EDITOR) · C4 templates.get workspace-scoped · C2 billing.*→OWNER · C3 bulk publish/unpublish removed (schema+service+router+UI dead branch) + sites.create/folders.create→EDITOR · M2 VIEWER-mutable resources gated via new `guardSiteRole` (forms submissions, site versions/components, user templates) + api-tokens→ADMIN + integrations.add→ADMIN · N20 createdBy stamped server-side · M3 team resolves via `resolveWorkspaceId` (active+ACTIVE). 6 commits `56de0d4d`..`13f44811` + 6 new authz regression tests; 355 server tests green; tsc 0; owner live-smoke clean. **Review-flow pair — ✅ DONE 2026-07-23** — M12 `resolveReview` rejects self-approval (resolver === requester → BAD_REQUEST, any role); M23 `submitReview` only rotates the client token when a clientEmail is present (internal dashboard submit no longer silently kills the client's live link) + Send-for-Review button hidden when `agency_layer` off. Commit `d9a5ec6d` + 3 regression tests (M12 block, M23 no-rotate, M23 client-invite rotate); review test suite 12/12. Committed via a filtered patch to isolate from a concurrent session's unrelated review-rounds WIP in the same file. Not pushed.
...
**Week 4 — honesty + IA polish: ✅ DONE 2026-07-23** — M19 real synchronous data export (JSON download, dead requestDataExport removed) · M24 API-tokens honesty note · M26 marketplace install honesty note · M25 theme-push preview (accurate blast radius via previewPush) + per-site Undo (rollback) wired onto the orphaned backends · M13 `/dashboard/activity` page (All/Mine/Team) + repointed home "View all" · M14 template detail shows page list instead of blank · N16/N17 a11y (scrollable DataTable + aria-labels + sr-only presence). 6 commits `7b663cac`..`6457bd5c`; 374 server tests green; tsc 0; all browser-verified (export 200 w/ real data, activity page, template pages, API-tokens note). Not pushed.

**All audit tiers (Week 1–4 + 3.5 + review-flow pair) complete. Backlog cleared.** Deliberately not built (need founder product calls, not audit fixes): whole-site AI removal (S2.1-2.3), rail convergence (M5 ship-plan), the deferred N-tier cosmetics (N4-N5 sparkline/health honesty, N23-N28 minor copy). None block GA.
