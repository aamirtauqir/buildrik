...
**Week 3 — states + dead ends: ✅ MOSTLY DONE 2026-07-22** — M1 error-as-empty (Domains/Redirects/Access/Analytics/API-tokens/media grid) + M11 account page → real ErrorState with retry · M7 confirms (unpublish, remove-domain, delete-asset) + N16 aria-labels · M9 silent failures (bulk onError, session-revoke try/finally, avatar-upload catch, + missing onError toasts on domain-remove/redirect-delete/share-revoke/token-revoke+delete/asset-delete) · M10 focus trap + restore in the shared Modal (fixes every dialog) · M21 AI poll-error surfaced · M22 media folder upload (pass folderId). 6 commits `343f560d`..`695ca5bc`; 355 server tests green; tsc 0; unpublish-confirm + billing + media browser-verified. **Still open this tier: M18 revoke-all-sessions dead (needs currentSessionId plumbed from server), M20 sole-owner delete guard (backend + UI props), M8 unsaved-changes dirty guards on forms.** Not pushed.
