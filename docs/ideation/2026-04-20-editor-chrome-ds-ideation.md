...
| **Week 1** | `shared/constants/layout.ts` (SSOT) + ESLint rule banning magic layout literals (WARN at baseline 566) + ESLint rule banning `editor/→components/` imports (error, 0 existing) + `.ds-green-panels.json` allowlist mechanism wired into gates. **Codemod deferred to Week 3 per-file migration** — mechanical bulk replacement of 177 TSX literals is risky without semantic context (a `44` could mean panel-header OR an unrelated row). Week 3 PanelShell migration has the context. | #3 (partial), #6 | 1 day | New layout literals can't land; layout constants ready for consumers; green-panel ratchet wired (0 files listed initially) |
