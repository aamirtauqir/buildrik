...
- [ ] A1 failing RTL test (crash seeded → banner + timestamp + actions; consume clears sentinel; Keep dismisses; no-crash → null; Discard clears localStorage + reloads)
- [ ] A2 fail · A3 implement + wire · A4 pass; tsc · A5 commit
...
- [ ] B1 failing tests (banner renders per kind; Sign in navigates; Retry reloads; hook calls onLoadError with the right kind; unwired → toast fallback)
- [ ] B2 fail · B3 implement + wire · B4 pass; tsc · B5 commit

### Slice C — Verify
- [ ] C1 editor vitest green for touched files; tsc gate PASS; ds-ssot green
- [ ] C2 manual/scripted behavioral check per master-plan acceptance (crash sentinel → banner restores; load-fail → persistent banner + retry)
