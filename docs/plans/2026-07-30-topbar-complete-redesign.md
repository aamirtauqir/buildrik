...
- [x] **T8 (P2) — FULLY SHIPPED 2026-07-30** (review pill label capped 140px + truncate + title; site name min-width 120 + flex-shrink — shrinks LAST. **The earlier deferral is closed:** tier 2 fires off `container-type: inline-size` on `.bk-topbar` + `@container (max-width: 1200px)`, which is not the `@media (prefers-*)` the DS restricts to a11y.css — and it keys off the bar's real width, not the viewport the shell's rails eat into. `.bk-save__ago` is its own element so the timestamp drops while "Saved" stays. D7 grammar landed with generated tokens, not T1: rule 4 text-first (saved/saving/unsaved plain muted, only offline/error/conflict tinted), rule 3 review neutral-unless-blocking on `--bk-bg-subtle`+`--bk-ink-soft` — the purpose-named `bg-pill-neutral` stays a Figma follow-up since tokens are generated, never hand-added. Rule 6 amber arbitration in StudioHeader (warning review pill demotes when an amber save AND amber chip already spend the budget). Tier 3 presence cap = 2, unconditional by choice: CSS can hide a third avatar but cannot re-count "+N", and a badge that disagrees with the faces beside it is worse than a tighter stack. **Not verified:** the 1152px fit itself is arithmetic from §7, not a measurement — jsdom cannot lay out, so it needs a live browser check.) — ui/shell — status grammar + compact modes
...
- [x] **T9 (P3) — SHIPPED 2026-07-30** (F22 confirmed by class, not by hope: ⌘, is the Preferences accelerator in Chrome, Safari AND Firefox on macOS — the page never sees it, exactly like ⌘H at F6. Hint is now platform-aware `SETTINGS_KBD` = ⌃, on mac / "Ctrl ," elsewhere, mirroring `HISTORY_KBD`; handler already took ctrl OR meta. **Second, larger bug found while verifying:** the chord opened the left panel's settings tab while the "Site settings" row it is printed on opened the project-settings modal — one advertised shortcut, two destinations. `useEditorShortcuts` gained an explicit `openSiteSettings` callback wired to `modals.openProjectSettings`, so row and chord are one contract; the stale header comment block now lists all three site-menu chords. 6 tests. **Still needs the live pass** in Chrome/Safari macOS — the OS/browser layer is the one thing jsdom cannot prove.) — shell — verify ⌘, chord delivery
