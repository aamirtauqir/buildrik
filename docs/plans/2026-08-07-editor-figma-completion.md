...
| Item | Gate |
|---|---|
| ~~Full tokenization (old T11)~~ **PULLED FORWARD + EXECUTED 2026-08-07 (same day)** — founder redirected the lane to Figma design. Results: **45 paint styles created** (all bound to variables); **5,673 raw paints bound** across 3 passes (3,370 exact + 2,123 drift-remap + 180 strays), 0 errors; product raw paints **6,075 → 235 (−96%)**. The drift-remap killed a second neutral ramp (#595966 family, ~1,500 paints) plus stray blues/greens/ambers (#3873f2, #26b273, #d98c26) that recent boards imported. Reference/Archive sections + hotspot frames deliberately excluded. Screenshot-verified on the 3 heaviest boards — no visual regressions. **Tail reviewed same evening carrier-by-carrier** (+88 binds): insertion indicators → accent, inspector element-chips → accent-tint/accent-text, changelog diff marks → success/warning, context-menu dividers → border, undo-toast text → gray/200, smart-guides mock blocks → blue ramp, and the banned `#7359d9` purple resolved to the **flowbite purple avatar allowlist** (semantics were right, the ramp was wrong). **Final: 147 raw paints left** (97.6% cleared) — all ≤5-instance mock-content one-offs, deliberately left. | **done** |
