...
- ~~First-run blank canvas → PageWizard mounts unless `buildrik:page-wizard-dismissed`/starter-seen (`AquibraStudio.tsx:140-161,582-588`).~~ — **stale, verified 2026-08-25**: `PageWizard` is 0 files / 0 refs. **§11.3 item 2 already said this on 08-23 while this line kept asserting it** — the chapter contradicted itself for two days. The shipped first-run surface is the empty-canvas CTA "Browse templates / Start blank".
- **Crash recovery — two separate mechanisms, not one.** Corrected 2026-08-25 after the first live walk of this leg (`docs/walks/F-A1-boot-and-project-load.md` addendum):
  - *Same session, at fault time:* `handleRuntimeFault` → `recoverFromInactivity()` re-checks active page, page root and selection validity and emits `CANVAS_FORCE_SYNC` (`RecoveryManager.ts:~130-190`). **This** is what restores page/root/selection.
  - *Across a reload:* the sessionStorage sentinel `buildrick:last-crash` carries only `{at, source, reason}`. Its sole consumer is `RecoveryBanner.tsx:47`, which consumes it in a `useState` initialiser on first render and shows **"Recovered your work after an unexpected close · <rel time> · N pages"** with `Discard & reload` / `Keep changes`. It restores nothing by itself.
  - The previous wording merged the two, which reads as "the sentinel restores your work". It does not; the local draft does, and the banner just tells you so.
  - `EVENTS.RUNTIME_FAULT_CAUGHT` is emitted (`:101`) with **zero subscribers** outside its own test.
...
- **Provider routing is not tier-based when `OLLAMA_BASE_URL` is set** — recorded 2026-08-25, first live fire of this pipeline (`docs/walks/F-A5-F-A8-ai-and-sync.md` addendum). `resolveModelForUser` (`quota.service.ts:167`) returns `"ollama"` on the **mere presence** of the env var, before tier or client hint is consulted. The contract line above ("Server-authoritative model: client model is only a hint, gated by tier") is true only when that var is unset.
- **The W3 fail-fast guard checks configuration, not reachability.** `assertProviderConfigured` (`ai.service.ts:481-491`) asks only whether `OLLAMA_BASE_URL` is non-empty. A configured-but-unreachable provider passes it and then fails deep as `INTERNAL_SERVER_ERROR` — the exact outcome its own docstring says it exists to prevent ("an opaque deep 401 … or a silent empty reply"). The panel renders **"The AI service didn't respond … usually the model provider, not your site · Unknown error"**, which misattributes a local misconfiguration to the vendor. Dev-scoped: `scripts/check-prod-env.mjs:242-251` hard-fails a production deploy that sets the var.
- **Verified working once routing is corrected** (2026-08-25): single-shot validated edit-command batch → diff rows with from/to → `Discard` / `Apply` → applied to the correct element out of a 50-element page scope → **one ⌘Z reverts it**, so *"Apply lands as one undo step"* is measured true, not just claimed.
...
6. Export: CSS (dark: media|data-attr|off) / JSON / Tailwind / ⛔ Figma stub. Import: JSON-only parser, conflict strategies Replace/keep-mine/keep-theirs. **Walked 2026-08-25** (`docs/walks/U3-brand-design-system.md`):
   - All four export formats render. Figma's stub names its own workaround — *"Coming soon — export JSON and use the Figma Variables importer"*.
   - ⚠ ~~Tailwind drops dark variants~~ understates it: the Tailwind export is **colors only** (`exportUtils.ts:121-123` filters `t.type === "color"`), so typography, spacing, radius, shadow and the rest never reach it, under a label that just says "theme.extend config".
   - The three dark strategies are three real branches (`CSSBundler.ts:71-86`) but all sit under `if (darkColorLines.length > 0)`, so on a project with `0 dark variants` they emit identically. Verified identical live; **not** verified that `media` and `data-attr` differ on a project that has dark values.
...
HistoryTab (H) → Changes/Saves views → scrub timeline → hover-preview → restore (F-A6) → autosave persists restored state. Auto-milestone banner surfaces suggested save points. Walk record: `docs/walks/U9-version-rescue.md`. **Two preconditions the flow text hides, both measured 2026-08-25:** the hover-preview and the Visual compare tab need `visualSnapshot`, and `VersionTimelineManager.ts:237` sets it to `null` for every auto-checkpoint on purpose — on a site of pure auto-saves both are inert by design, not broken. The scrub timeline previews from the nearest NamedVersion's snapshot for the same reason, so it too has no frames there. Server + local prune caps are a matched pair at 50, evicting auto-saves only (`site-version.service.ts:71`, `VersionHistoryStorage.ts:223`) — read, not exercised. ⛔ Scrubber, AI summary and the Published/What's-live tab remain unwalked. **The eight `Compare · *` boards belong to the REVIEW flow, not to this one** — `ApprovedCompareView.tsx:149` renders their three modes verbatim, prints their `N changes · M of N` stepper and filters by page, and `ReviewTab.tsx:465` mounts it; board `168:2` labels its left pane `v3 · approved` and hotspots to `Compare · resend-confirm`. A 2026-08-25 note here claimed the app diverged from them; it had matched them against version history's own `CompareView`, which is a different component. ⛔ Version-history Compare (two-pill Visual/Semantic + screenshot diptych) has **no board in the History family's 23** — unboarded surface, founder decision pending.
...
~~Rail Site (S)~~ → **⌘K "Open Settings panel · S"** or the site menu (there is no Site rail tab — the rail is Insert/Layers/Pages/Media/Content/Brand), opening **full-screen**, not in the sidebar drawer. SettingsTab drill-in (root ⇄ section, 180ms lock + dirty guard): ~~10 sections in 3 groups + 3 workspace deep-links (Domains/Members/Billing)~~ — **counted live 2026-08-24: 13 sections** in SITE (General/Branding/SEO), DISTRIBUTION (Export/Domains/Analytics/Localization) and PLUMBING (Custom code/Redirects/Headers/Forms/Integrations `Pro`/Webhooks), **plus 2 workspace deep-links** (Members/Billing) under a group labelled "opens dashboard ↗". **Domains moved into the editor's own DISTRIBUTION group** and is no longer a deep-link. Walk record: `docs/walks/U12-site-settings.md`. Central dirty counter + sticky savebar. Plan gates: advanced + integrations require pro → LockedScreen → billing. ⛔ Redirects/Headers **and Forms** saved but not enforced/captured on live sites (explicit banners, each naming the gap, the consequence and the condition); ~~Integrations all "Coming Soon"~~ — **not verifiable on a free workspace, 2026-08-25**: Integrations renders the Pro gate ("Available in Pro" + Upgrade Now) and its contents sit behind it; Localization routing "Phase D" (§13b B8).
