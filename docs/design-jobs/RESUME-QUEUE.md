...
| FIG-CO-17 | the 9 Compare boards | the header exists (`ReviewTab.tsx:500-504`) but the boards draw the wrong one: copy is `Compare with approved` not `Compare`, the left control is a **Back** button no board draws, every board draws a `✕` that exists in neither toolbar, and the boards order page-select before the count where the code renders count first | one pass over the family: retitle, swap Back in for ✕, swap the count/select order. Not normalisable from inside a single-board repair |
