...
const HOPS = [
  ...
  { from: "Records", to: "Dynamic page", ok: false, conf: "verified",
    ...
    body: "PARTLY BROKEN \u2014 and this hop said something FALSE for one build, on my own bad grep. The claim was 'the link does not exist in the editor at all', from a search for `dynamicPages` that returned zero. The codebase spells it `dynamic-pages`. The screen is fully wired: useContentPanel.ts:28 declares the view kind, ContentViews.tsx:261 draws the 'Dynamic pages \u203a' row, ContentTab.tsx:256 supplies the handler and :299-317 renders DynamicPagesView. This same arc drew four boards for that screen \u2014 the hop was refuting its own file.\nWhat IS true, and is the whole gap: nothing in that panel sets `pageTemplatePath`. ContentViews.tsx:630 says so in the code's own words, and the only writer in either tree is the create-collection wizard. And the Pages panel carries no CMS reference of any kind, so the routes a collection will generate appear in no page list."
  },
  ...
  { from: "Preview", to: "SEO / sitemap", ok: false, conf: "verified",
    ...
    body: "BROKEN \u2014 and this hop was wrong TWICE before it was right. It first said 'a publish emits no sitemap'; a publish does (lib/publish-files.ts:82-84), and only the ZIP and single-file EXPORTS go without. It then said 'no canonical, OG or twitter tags are injected at all'; SEOInjector.ts:141-150 injects canonical, og:locale, og:type, og:title and a twitter card unconditionally, through ExportEngine.ts:486.\nThe true defect is worse than either version. cms.service.ts:208-215 clones the TEMPLATE page's whole head into every generated record page, so each one carries a second <title> AND the template's canonical and og:title \u2014 N pages all claiming to be one URL. That is a self-canonicalisation bug no amount of 'the tags are missing' would have found."
  },
