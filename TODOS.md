...
- [ ] **[LOW] Three pre-existing test failures** unrelated to these fixes, seen in the full run: `team-components.test.ts` asserts a `MEMBER_ACTIONS` export that `member-actions.tsx` does not have, `settings-rail-routes.test.ts` expects agency routes and gets `[]`, and `template-ai-components.test.ts` fails at import. None of their subjects appear in the three fix commits.
