...
const SelectionBoxOverlayComponent: React.FC<SelectionBoxOverlayProps> = ({
  ...
  // Check if element is locked — hides resize/rotation handles
  const isElementLocked = React.useMemo(() => {
    if (!composer || !elementId) return false;
    return composer.elements.getElement(elementId)?.isLocked() ?? false;
  }, [composer, elementId]);
