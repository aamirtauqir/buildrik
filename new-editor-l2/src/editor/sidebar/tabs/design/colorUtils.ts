...
export function wcagTooltip(level: WcagLevel): string {
  switch (level) {
    case "aaa":      return "Passes AAA — excellent contrast (≥7:1) for all text sizes";
    case "aa":       return "Passes AA — meets minimum contrast (≥4.5:1) for normal text";
    case "aa-large": return "Passes AA for large text only (≥3:1). Use 18px+ or bold text";
    case "fail":     return "Fails WCAG — contrast too low. Avoid using this color combination";
    case "na":       return "Cannot measure — background has transparency";
  }
}
