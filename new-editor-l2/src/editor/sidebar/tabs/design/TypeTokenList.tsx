/**
 * TypeTokenList v10 — Type tab pane
...
import type { ResponsiveMode } from "./useTypeTokens";
...
const TypeScaleRow: React.FC<TypeScaleRowProps> = ({ token, onChange }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ width: 64, flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 9, color: "var(--aqb-text-muted)", marginTop: 1 }}>
          {token.id}
        </div>
      </div>
...
const FONT_OPTIONS = [
  "Inter", "Roboto", "DM Sans", "Geist", "Montserrat",
...
const FontFamilyRow: React.FC<FontRowProps> = ({ token, onChange }) => (
  ...
  >
    ...
    >
      ...
      {!FONT_OPTIONS.includes(token.value) && (
        <option value={token.value}>{token.value}</option>
      )}
    </select>
  </div>
);
...
const TypePreviewBand: React.FC<TypePreviewBandProps> = ({ fontTokens, sizeTokens, responsiveMode }) => {
  ...
  const getSize = (id: string) => {
    ...
    // Mobile: scale down by 85%
    if (responsiveMode === "mobile") {
      const size = parseFloat(token.value);
      return `${Math.round(size * 0.85)}px`;
    }
    return token.value;
  };
  ...
  return (
    ...
    >
      <div style={{ fontFamily: headingFont, fontSize: getSize("font-size-4xl"), fontWeight: 700, color: "var(--aqb-text-primary)", lineHeight: 1.1 }}>
        Heading 1
      </div>
      <div style={{ fontFamily: headingFont, fontSize: getSize("font-size-3xl"), fontWeight: 600, color: "var(--aqb-text-primary)", lineHeight: 1.2 }}>
        Heading 2
      </div>
      <div style={{ fontFamily: bodyFont, fontSize: getSize("font-size-base"), color: "var(--aqb-text-secondary)", lineHeight: 1.6 }}>
...
export const TypeTokenList: React.FC<TypeTokenListProps> = ({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      {fontTokens.length > 0 && (
        <>
          <div style={SECTION_HEADER}>Font Families</div>
      ...
      {sizeTokens.length > 0 && (
        <>
          <div style={SECTION_HEADER}>Type Scale</div>
          {sizeTokens.map((token) => (
            <TypeScaleRow key={token.id} token={token} onChange={onTokenChange} />
          ))}
        </>
      )}
    </div>
