...
function MediaTabWithComposer({
  ...
  return (
    ...
    >
      ...
      {/* Zone 3: Source bar — pill toggle */}
      <div className="med-source-bar">
        <div className="med-source-bar-inner" role="tablist" aria-label="Media source">
          <button
            role="tab"
            className={`med-src-btn${!isDisc ? " active" : ""}`}
            onClick={() => handleSetSource("mine")}
            aria-pressed={!isDisc}
            aria-selected={!isDisc}
          >
            My Library
          </button>
          <button
      ...
      {/* Zone 5: Search — Library only; hidden on Discovery to remove false affordance */}
      {!isDisc && (
      ...
      {isDisc ? (
        ...
        <OnboardingEmptyState
          ...
          onBrowseStock={() => handleSetSource("disc")}
        />
      ) : (
        <LibraryView
