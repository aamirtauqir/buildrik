...
- **Icons:** 20px Lucide, `--aqb-text-secondary`
- **Active icon:** `--aqb-primary` color + 2px left border accent
...
- **Tab label:** `--aqb-body-sm`, `--aqb-text-secondary`
- **Active tab:** `--aqb-text-primary`, 2px `--aqb-primary` bottom border
- **Hover:** `--aqb-text-primary`
...
- **Segments:** `--aqb-body-sm`, `--aqb-text-secondary`, separated by " > "
- **Hover:** Segment becomes `--aqb-primary`, cursor pointer
- **Click:** Selects that ancestor element
- **Truncation:** If path > 4 segments, collapse middle with "..." popover

## Context Menu
- **Width:** 240px
- **Background:** `--aqb-chrome-surface`, `--aqb-elevation-2`, `--aqb-radius-md`
- **Items:** 32px height, `--aqb-body` text, `--aqb-text-secondary` for keyboard shortcuts (right-aligned)
...
- **Disabled items:** `--aqb-text-tertiary`, no hover effect

## Command Palette (⌘K)
- **Width:** 480px, centered horizontally, top-third vertically
- **Background:** `--aqb-chrome-surface`, `--aqb-elevation-3`, `--aqb-radius-xl`
- **Search input:** Full width, 44px height, auto-focus, `--aqb-heading-md` text
- **Results:** List below, 36px per item, fuzzy-matched characters highlighted in `--aqb-primary`
- **Active result:** `--aqb-primary-subtle` bg
...
- **Background:** `--aqb-chrome-bg`
- **Tabs:** `--aqb-body-sm` text, `--aqb-text-secondary`. Active: `--aqb-text-primary` + 2px `--aqb-primary` bottom border.
