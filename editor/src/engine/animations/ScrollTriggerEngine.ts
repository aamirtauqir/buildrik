...
export class ScrollTriggerEngine {
  ...
  createScrollTrigger(config: ScrollTriggerConfig): ScrollTriggerInstance | null {
    ...
    // Create ScrollTrigger — disable scrub when reduced motion is preferred
    // so animations jump to their end state instead of scrubbing
    const scrollTrigger = ScrollTrigger.create({
      trigger: triggerElement,
      start: config.start,
      end: config.end,
      scrub: prefersReducedMotion ? false : config.scrub,
      pin: config.pin,
