...
describe("Sidebar Navigation", () => {
  it("SIDEBAR_NAV_ITEMS has exactly 6 items", async () => {
    const { SIDEBAR_NAV_ITEMS } = await import("@/components/dashboard/sidebar");
    expect(SIDEBAR_NAV_ITEMS).toHaveLength(6);
  });
  it("nav items match PRD Section 3.1 (+ Media)", async () => {
    const { SIDEBAR_NAV_ITEMS } = await import("@/components/dashboard/sidebar");
    const labels = SIDEBAR_NAV_ITEMS.map((item: { label: string }) => item.label);
    expect(labels).toEqual(["Dashboard", "My Sites", "Media", "Team", "Billing", "Settings"]);
  });
  it("nav items have correct href paths", async () => {
    const { SIDEBAR_NAV_ITEMS } = await import("@/components/dashboard/sidebar");
    const hrefs = SIDEBAR_NAV_ITEMS.map((item: { href: string }) => item.href);
    expect(hrefs).toEqual(["/dashboard", "/dashboard/sites", "/dashboard/media", "/dashboard/team", "/dashboard/billing", "/dashboard/settings"]);
  });
