...
> **Ramp change, 2026-08-28 — founder-authorized.** `--text-page-title` 24→20
> and the hero stat metric 27/730→22/700 were changed on the founder's
> instruction ("text kabhi bara hai" — the text is sometimes too big), then this
> section was updated to match. Recorded explicitly because editing the spec to
> ratify a change already in the diff is how a design system stops being an
> authority. `--text-section-title` went 15→14 in the same pass and was **put
> back to 15**: `--text-body` is 14, so headings and body copy had become the
> same size separated by weight alone — a step of zero, which the founder never
> asked for.

**Token contract** (`globals.css @theme`): radius scale `--radius-xs 4 / sm 6 / md 8 / lg 8 / xl 12 / pill` (lg 10→8 with the Flowbite migration 2026-07-30); `--font-mono` Geist Mono + `tabular-nums` for **tabular data** (counts, $, sizes, seats, dates — via `MetricValue`) — **hero stat metrics may use the display font**, as the dc artifact does; consistency within a surface matters more than mono everywhere. (That clause said "(Inter Tight)" until 2026-08-28; the dashboard moved off Inter Tight on 2026-07-26 and the display font is Inter — the sentence was edited twice without the stale parenthetical being noticed either time.) Named text tokens `--text-page-title 20 / section 15 / metric 22 / eyebrow 11 / body 14 / body-sm 12` (one ramp — Tailwind's `text-sm`/`text-xs` are swept out of the dashboard) are **preferred**, but **artifact-matched pixel values** (`text-[13.5px]`, `font-[520]`) are allowed when matching a mockup 1:1. Depth is hairline borders + subtle card shadow (`shadow-card`), not heavy elevation.
...
These primitives are **compositions of Flowbite, not an alternative to it** — the dashboard's UI system is Flowbite (`packages/dashboard/AGENTS.md` §UI system). Precedence: primitive → `flowbite-react` directly → a new primitive built from flowbite-react. Four of the thirteen compose flowbite (button, data-table, pill, progress-bar). This said six and included `modal` until 2026-08-27 — `modal.tsx` mentions flowbite-react only in a comment explaining why it rejects it. The other nine are not a drain target: seven have no flowbite counterpart, and `input-field` would lose documented behaviour by converting (see packages/dashboard/AGENTS.md §UI system). So "do NOT hand-roll" and "Flowbite first" say the same thing: a screen should never reach raw HTML for a control.
...
3. **`border-radius` ≤ 4px on panel chrome containers.** Panels, headers, toolbars, footers, sidebar rows, inspector sections, rail zones all cap at `--buildrick-radius-sm` (4px). **Form atoms and overlays are exempt** (see tier above) — a modal, dialog, popover, toast or form atom is not a panel chrome container and keeps the full radius scale (8 on buttons/inputs/cards, 12 on modals). *The rule is split by **surface**, not by number* (founder call G13, 2026-09-02).

   This exemption has to be maintained in the gate as well as here, and it was not: every path in `ds-grep-gates.sh`'s `FORM_ATOM_EXCLUDE` named `shared/ui/…`, a directory that stopped existing when the form atoms moved to `editor/chrome-ui/`. The list matched nothing, Gate 13 counted 19 modal/overlay declarations against the chrome budget, and any modal adopting the boards' r12 pushed the count over baseline — which is why the r12 review modal (1172:4840) and the r6 starter swatch (152:137) were both reverted *after* they landed. The written rule was right for months while the gate enforced the opposite. **When a component file moves, move its name in `FORM_ATOM_EXCLUDE`/`OVERLAY_EXCLUDE` in the same commit**, and plant a violation to watch the gate fail before believing a PASS.
4. **Editor chrome: 20px minimum hit target; the glyph inside it may be smaller.** The eye/lock controls on a Layers row draw a 10×10 glyph inside a 20px box (`layers-v2.css:162-168`), and that is the contract — a board that draws only the 10×10 glyph is describing the mark, not the target. Rest colour is `--bk-ink-muted` (`#6B7280`): gray-300 on white is **1.47:1** and cannot satisfy 1.4.11. (Rule 413 below covers marketing only; this line covers the editor. Founder call G3, 2026-09-02.)
5. **No decorative hover effects on panel chrome** — no glow, no scale, no rotate, no tint animations on panel containers. Hover on rows uses a background-color change to a hover-surface token. Form-atom hover states (button color shift, input focus ring) are unchanged.
