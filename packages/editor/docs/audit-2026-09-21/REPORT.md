...
- Built on the owner's go (defaults in 04 §2): **40 boards (+2 after the decisions closed: CTA verbs, client pins)** in a new section `v3 · Code-only features` (`7563:197895`) — 33 committed (P0 17 · P1 16) + 7 waterfall — a launcher wired into the STATES index, **11 entry-point edits** (Comments toggle on the topbar master, propagated to 503 live boards; view-mode and collaboration rows in the site menu; Reopen; Mark all read; four context-menu rows; shortcut legend; toast Undo; CMS Conditions; copy fixes; Brand hex/Light-Dark; Detach/Listings), **28 annotation cards** on Figma-only controls, and **42 hides** of already-orphaned clones.
...
- Traceability: all 1,020 code rows carry a Phase-19 status, **0 ✗ Missing** after the owner closed all 36 decisions on 2026-09-21 (two more boards built: CTA verbs, client pins). All 1,140 Figma rows carry one of the four statuses; 114 are ✗ Dummy (annotated or hidden), 612 △ Partial.
...
**All 36 owner decisions were closed on 2026-09-21** (05 "Owner decisions closed"): the Figma-side consequences are applied (4th publish-gate route on 523 shells + `reviewChangesRequested`, CTA-verbs and client-pin boards, 14 copy/visibility edits), the code-side consequences are Tier 3 rows in the plan doc §16. Headline deltas: publish confirm (Figma deploys on click; code confirms) · conflict dialog (2 vs 3 actions) · orphan comments (announcement vs reattach picker) · notifications scope (site vs workspace) · viewer feedback (located pins vs notes) · review sending (inline email vs 3-field popover + link modal) · Brand: four designs · delete pattern (instant + Undo vs confirm modal) · page-settings save (autosave vs Done/Cancel) · shortcut legend prints unbound chords · typed-DELETE policy · export scope copy. Owner decisions in 03 §E (39) and 04 §2 (15 with defaults).
...
`07-traceability.md`: code 1,020 → ✓ Represented 672 · ✓ Combined 152 · ✓ Internal 81 · ✓ Planned for later 61 · ✓ Deprecated 32 · ✓ Hidden 22 · **✗ Missing 0** (the two owner-gated rows became C-01 / C-03 once the decisions closed); Figma 1,140 (1,098 + 42) → ✓ Code 406 · △ Partial 612 · ✗ Dummy 114 · ○ Planned 8.
...
| 17–18 | Traceability both ways | ✓ 07 (0 ✗ Missing after the 2026-09-21 decisions) |
