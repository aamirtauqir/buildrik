...
- ☑ **Real-time collaboration** — built over a DB-backed SSE transport (no separate WebSocket server needed; uses the existing serverless+DB stack, same pattern as the notifications SSE). CollabOperation model + migration; `collab.service` + `POST /api/collab/[siteId]/ops` + `GET /api/sse/collab/[siteId]`; client `SSETransport`; `CollaborationManager.startSession`; outbound wired via `recordAndMaybeBroadcast` (undo path untouched); "Start collaboration session" command. Unit-tested; end-to-end needs the migration applied + two live clients to smoke-test. (A WebSocket transport can replace SSE later for lower latency — same interface.)

ALL 5 Tier C features now have implementations. Remaining work on these 3 is LIVE SMOKE-TESTING (real AI keys / real domain+DNS / two live collab clients + `prisma migrate deploy`), not code.
