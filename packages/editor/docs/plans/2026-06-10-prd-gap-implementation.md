...
- ☐ B1 Transfer-ownership UI (backend done)
- ☐ B2 Email-change UI (backend done)
- ☐ B3 Save-as-custom-template wiring (`openSaveTemplate` zero callers)
- ☐ B4 CMS records management UI (engine CRUD orphaned) — large
- ☐ B5 Multi-select copy/paste/duplicate (only delete is multi-aware)
- ☐ B6 Heading H1–H6 level switcher; select-options editor
