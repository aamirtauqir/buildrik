...
| Token | Hex | Usage |
|-------|-----|-------|
| `--aqb-primary` | `#2563EB` | CTAs, active states, selection |
| `--aqb-primary-hover` | `#3B82F6` | Hover on primary elements |
| `--aqb-primary-active` | `#1D4ED8` | Active/pressed |
| `--aqb-primary-subtle` | `rgba(37,99,235,0.08)` | Selected rows, hover backgrounds |
| `--aqb-primary-glow` | `rgba(37,99,235,0.25)` | Focus rings, selection glow |

### Semantic
| Token | Hex | Pairs With | Usage |
|-------|-----|-----------|-------|
| `--aqb-success` | `#22C55E` | `--aqb-success-subtle` `rgba(34,197,94,0.1)` | Published, save confirmed, SEO good |
| `--aqb-warning` | `#F59E0B` | `--aqb-warning-subtle` `rgba(245,158,11,0.1)` | Unsaved, SEO needs work, AI medium |
| `--aqb-error` | `#EF4444` | `--aqb-error-subtle` `rgba(239,68,68,0.1)` | Delete, SEO poor, save failed |
| `--aqb-info` | `#3B82F6` | `--aqb-info-subtle` `rgba(59,130,246,0.1)` | Informational, AI suggestion |

### Text
| Token | Hex | Usage |
|-------|-----|-------|
| `--aqb-text-primary` | `#F1F3F9` | Primary text on dark chrome |
| `--aqb-text-secondary` | `#9CA3B4` | Muted text, labels, timestamps |
| `--aqb-text-tertiary` | `#6B7280` | Placeholders, disabled |
| `--aqb-text-inverse` | `#0F1117` | Text on white/light surfaces |
...
- Use `--aqb-primary-subtle` for hover backgrounds, not raw opacity hacks
- Use `--aqb-text-secondary` for non-essential text, keeping `--aqb-text-primary` for actionable content

**Don't:**
- Use `--aqb-primary` for destructive actions (use `--aqb-error` instead)
