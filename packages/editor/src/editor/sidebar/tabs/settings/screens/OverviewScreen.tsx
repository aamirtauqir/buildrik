...
/**
 * `settingsOverviewSchema` from `packages/shared/schemas/site-detail.ts`
 * (phase1-brief.md §Contracts), written out here because that export lands
 * with B's branch. At merge: `import type { SettingsOverview } from
 * "@buildrik/shared/schemas/site-detail"` and delete this block.
 */
export interface SettingsOverview {
  site: { name: string; defaultLocale: string; plan: "FREE" | "PRO" | "BUSINESS" };
  general: { siteName: string; language: string };
  localization: { locales: number; notStarted: string[] };
  seo: { allowIndexing: boolean; robotsTxtSet: boolean };
  domains: { primary: string | null; pendingDns: number };
  redirects: { rules: number; suggestions: number };
  analytics: { providers: string[]; receiving: boolean };
  forms: { forms: number; submissions: number };
  customCode: { head: boolean; body: boolean; css: boolean };
  headers: { csp: boolean; hsts: boolean };
  integrations: { connected: number; available: number };
  webhooks: { endpoints: number; lastDelivery: "ok" | "failed" | null };
  members: { used: number; seats: number };
  billing: { plan: string; priceMonthly: number };
  attention: Array<{
    kind: "locale-not-started" | "dns-pending" | "webhook-failed";
    title: string;
    detail: string;
    /** The nav id `Open ›` goes to. */
    section: string;
  }>;
}

/* The procedure is B's; until its branch merges, `AppRouter` does not carry
   it, so the call is typed by hand against the brief's contract. The tRPC
   client is a proxy, so a server without the procedure answers with an error
   and the load-error card — no branch in here pretends otherwise. Drop the
   cast at merge. */
type SettingsOverviewProcedure = { query: (input: { siteId: string }) => Promise<SettingsOverview> };
function fetchOverview(siteId: string): Promise<SettingsOverview> {
  const siteDetail = getBuildrikClient(DASHBOARD_URL).siteDetail as unknown as {
    settingsOverview: SettingsOverviewProcedure;
  };
  return siteDetail.settingsOverview.query({ siteId });
}
