...
describe("useInspectorState — tab routing via config only", () => {
  it("returns a tab for a heading element", () => {
    ...
    expect(["layout", "appearance", "effects", "settings"]).toContain(result.current.activeTab);
  });
  ...
  it("setActiveTab updates the active tab", () => {
    ...
    act(() => result.current.setActiveTab("appearance"));
    expect(result.current.activeTab).toBe("appearance");
  });
