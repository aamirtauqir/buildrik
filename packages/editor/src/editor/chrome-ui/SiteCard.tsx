...
const BASE =
  "tw:flex tw:flex-col tw:border tw:border-[var(--bk-gray-200)] tw:rounded-lg tw:bg-white tw:overflow-hidden " +
  "tw:cursor-pointer tw:p-0 tw:text-left tw:[transition:var(--bk-transition-fast)] " +
  "tw:hover:border-[var(--bk-gray-300)] tw:hover:[box-shadow:var(--bk-shadow-raised)] " +
...
export function SiteCard({ name, state, stateLabel, meta, thumbnail, className, ...rest }: SiteCardProps) {
  return (
    <button type="button" className={[BASE, className].filter(Boolean).join(" ")} {...rest}>
      <span className="tw:aspect-[16/10] tw:bg-[var(--bk-gray-100)] tw:border-b tw:border-[var(--bk-gray-200)]">
