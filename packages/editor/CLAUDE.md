...
| Vibcoder primitive CSS (atoms/molecules/organisms/layouts) | `src/themes/components/` | CANONICAL — **generated, never hand-edit** |
...
4. Vibcoder mein nahi hai aur composition bhi nahi? → STOP. File issue / request vibcoder addition. Don't create parallel primitive.
...
- New `--buildrick-color-*` shape outside `themes/components/` → blocked by Gate 21.
...
- Hand-edit in `themes/components/` or `editor/shared/vibcoder/` → REJECT (regenerated by `npm run vibcoder:vendor`).
