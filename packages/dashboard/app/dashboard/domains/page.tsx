        ...
        <>
          <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="rounded-xl border p-4" style={{ borderColor: "var(--color-border-default)" }}>
              <p className="text-xs uppercase tracking-wide text-neutral-500">Connected</p>
              <p className="mt-1 text-2xl font-bold" style={{ color: "var(--color-text-primary)" }}>{connected}</p>
            </div>
            <div className="rounded-xl border p-4" style={{ borderColor: "var(--color-border-default)" }}>
              <p className="text-xs uppercase tracking-wide text-neutral-500">Verifying</p>
                  ...
                  return (
                    <tr key={d.id} className="border-b last:border-0" style={{ borderColor: "var(--color-border-default)" }}>
                      <td className="px-4 py-3 font-medium" style={{ color: "var(--color-text-primary)" }}>
                        {d.domain}
                        {d.isPrimary && <span className="ml-2 rounded bg-neutral-100 px-1.5 py-0.5 text-xs text-neutral-500">primary</span>}
                      </td>
                      <td className="px-4 py-3">
                        <Link href={`/dashboard/sites/${d.siteId}/domains`} className="text-[var(--color-primary)] hover:underline">{d.siteName}</Link>
                      </td>
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium" style={{ color: s.color, backgroundColor: s.bg }}>
                      ...
                      <td className="px-4 py-3">
                        {sslOk ? (
                          <span className="inline-flex items-center gap-1 text-xs text-[var(--color-success)]"><ShieldCheck className="h-3.5 w-3.5" /> Secured</span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-xs text-neutral-400"><ShieldAlert className="h-3.5 w-3.5" /> Pending</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {d.status === "VERIFIED" && (
                          <a href={`https://${d.domain}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-neutral-500 hover:text-[var(--color-primary)]">
