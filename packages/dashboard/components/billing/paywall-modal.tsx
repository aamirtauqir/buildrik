...
export function PaywallModal({ open, onClose, feature, description, unlockPlan = "Pro", perks = DEFAULT_PERKS }: PaywallModalProps) {
  return (
      ...
      footer={
        <>
          <Link
            ...
            className="flex-1 rounded-lg px-4 py-2.5 text-center text-body font-semibold text-white"
          ...
          <button
            onClick={onClose}
            className="rounded-lg border px-4 py-2.5 text-body font-medium"
    ...
    >
      <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-body-sm font-semibold" style={{ backgroundColor: "var(--color-primary-subtle)", color: "var(--color-primary)" }}>
        <Sparkles className="h-3.5 w-3.5" /> {unlockPlan} feature
      </span>

      <p className="mt-3 text-body" style={{ color: "var(--color-text-secondary)" }}>{description}</p>

      <ul className="mt-4 space-y-2">
        {perks.map((p) => (
          <li key={p} className="flex items-center gap-2 text-body" style={{ color: "var(--color-text-primary)" }}>
