  ...
  if (!isLoading && formBlocks.length === 0) {
    return (
      <div className="rounded-lg border border-dashed p-10 text-center" style={{ borderColor: "var(--color-border-default)" }}>
        ...
        {/* Since a10f19233 the publish worker points every action-less form
            at this site's form endpoint, so a published Form block does feed
            this inbox. Same sentence as the editor's Settings → Forms screen. */}
        <p className="mt-0.5 text-body" style={{ color: "var(--color-text-secondary)" }}>
          Publish a page with a Form block and its submissions arrive here.
        </p>
      </div>
    );
  }
          ...
          <DataTable
            ...
            empty={
              <div className="rounded-lg border border-dashed p-10 text-center" style={{ borderColor: "var(--color-border-default)" }}>
                ...
                {/* Same sentence as the editor's Settings → Forms screen — a
                    published Form block posts to this site's form endpoint. */}
                <p className="mt-0.5 text-body" style={{ color: "var(--color-text-secondary)" }}>
                  {filterFormBlockId
