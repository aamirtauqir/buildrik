...
describe("reserveQuota", () => {
  it("reserves when an existing row is under the limit (atomic increment hits)", async () => {
    ...
    const r = await reserveQuota("u1", "claude-haiku-4-5");
  ...
  it("never rejects an UNLIMITED (BUSINESS) plan", async () => {
    ...
    const r = await reserveQuota("u1", "claude-opus-4-7");
    expect(r.ok).toBe(true);
  ...
  it("propagates non-unique DB errors (no swallow)", async () => {
    ...
    await expect(reserveQuota("u1", "claude-haiku-4-5")).rejects.toThrow(
      "DB down",
    );
  });
});
...
describe("resolveModelForUser (server-authoritative)", () => {
  it("ignores a premium hint from a FREE user, falling back to the tier default", async () => {
    asPlan("FREE");
  ...
  it("lets a BUSINESS user use the premium model", async () => {
    asPlan("BUSINESS");
    await expect(resolveModelForUser("u1", "claude-opus-4-7")).resolves.toBe(
      "claude-opus-4-7",
    );
