...
export const authRouter = router({
  ...
  /**
   * Email-first flow prerequisite: reveals whether an account exists for the given
   * email and what auth methods are available (password / which OAuth providers).
   *
   * This endpoint is intentionally an "oracle" — the email-first UX requires it.
   * Mitigations:
   *   1. Strict rate limit (5 / 15 min per IP) — bulk enumeration is infeasible.
   *   2. Constant-time floor (200 ms) — timing attacks cannot distinguish a DB hit
   *      from a miss, and cannot probe cache behavior.
   */
  checkEmail: strictRateLimit
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Enforce minimum response time regardless of DB speed.
