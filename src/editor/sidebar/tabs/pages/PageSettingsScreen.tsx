...
export const PageSettingsScreen: React.FC<PageSettingsScreenProps> = ({
  ...
  const isDirty = React.useCallback((): boolean => {
    ...
    const origNoIndex = page.seo?.noIndex ?? false;
    const origNoFollow = page.seo?.noFollow ?? false;
    return (
      seoTitle !== origTitle || seoDesc !== origDesc || slug !== origSlug ||
      ogTitle !== origOgTitle || ogDesc !== origOgDesc || customHead !== origHead ||
      noIndex !== origNoIndex || noFollow !== origNoFollow || visibility !== "live"
    );
  }, [page, seoTitle, seoDesc, slug, ogTitle, ogDesc, customHead, noIndex, noFollow, visibility]);
  ...
  const handleSave = () => {
    ...
    // Slug format validation — only lowercase letters, numbers, hyphens, forward slashes
    if (trimmedSlug && !/^[a-z0-9][a-z0-9\-/]*[a-z0-9]$/.test(trimmedSlug) && trimmedSlug.length > 1) {
      addToast({ message: "Slug must contain only lowercase letters, numbers, hyphens, or slashes.", variant: "error" });
      return;
    }
  ...
  // ⌘S / Ctrl+S — save while drawer is open (ref keeps handler current without deps churn)
  ...
  const seoScoreColor = seoScore >= 70 ? "var(--pg-live, #22C55E)" : seoScore >= 40 ? "var(--pg-hidden, #F59E0B)" : "var(--pg-error, #EF4444)";
  const titleCcClass = seoTitle.length > 60 ? "pages-drawer__cc--err" : seoTitle.length > 50 ? "pages-drawer__cc--warn" : "";
  const descCcClass = seoDesc.length > 160 ? "pages-drawer__cc--err" : seoDesc.length > 140 ? "pages-drawer__cc--warn" : "";
  const previewTitle = seoTitle.trim() || page?.name || "Untitled";
  const previewDesc = seoDesc.trim() || "No description — add a meta description.";
  const previewSlug = slug.trim() || page?.slug || page?.name.toLowerCase().replace(/\s+/g, "-") || "";
        ...
        {page && (
          <>
            ...
            <div className="pages-drawer__hdr">
              ...
              <button
                className={`pages-drawer__save${isDirty() ? " pages-drawer__save--dirty" : ""}`}
                onClick={handleSave}
              >
                Save
              </button>
            ...
            <div className="pages-drawer__body aqb-scrollbar">
              ...
              {tab === "seo" && (
                <div className="pages-drawer__section">
                  ...
                  <div className="pages-drawer__field">
                    ...
                    <div className="pages-drawer__field-hint">Lowercase letters, numbers, hyphens, and slashes only</div>
                  </div>
              ...
              {tab === "advanced" && (
                <div className="pages-drawer__section">
                  {/* Visibility */}
                  <div className="pages-drawer__section-title">Visibility</div>
                  ...
                  <div className="pages-drawer__tog-row">
                    <div className="pages-drawer__tog-info">
                      ...
                      <div className="pages-drawer__tog-sub">Published but not shown in nav menus</div>
                    </div>
                    <button
                  ...
                  {visibility === "hidden" && <div className="pages-drawer__warn">Navigation visibility is coming soon — this value is not saved yet.</div>}

                  <div className="pages-drawer__tog-row">
                    <div className="pages-drawer__tog-info">
                      <div className="pages-drawer__tog-name">Password protect</div>
                      <div className="pages-drawer__tog-sub">Visitors need a password to access</div>
                    </div>
                    <button
                  ...
                  {visibility === "password" && (
                    <div className="pages-drawer__field" style={{ marginTop: 6 }}>
                      <div className="pages-drawer__warn">
                        Password protection is coming soon — this value is not saved yet.
                      </div>
                      <div className="pages-drawer__pw-row">
                        <input
                          className="pages-drawer__input"
                        ...
                        />
                        <button
                          className="pages-drawer__pw-eye"
                          type="button"
...
const SeoCheck: React.FC<{ ok: boolean; label: string }> = ({ ok, label }) => (
  <div
    style={{
      ...
      gap: 6,
      fontSize: 10,
      color: ok ? "var(--pg-live, #22C55E)" : "var(--aqb-text-muted)",
    }}
  >
    <span
      style={{
        ...
        height: 6,
        borderRadius: "50%",
        background: ok ? "var(--pg-live, #22C55E)" : "var(--aqb-border)",
        flexShrink: 0,
      }}
    />
    {label}
