...
export interface TemplatesTabProps {
  ...
  onTemplateUsed?: () => void;
  /** @deprecated No longer called — kept for API compatibility */
  onPagesOpen?: () => void;
...
export const TemplatesTab: React.FC<TemplatesTabProps> = ({
  ...
  function handleProgressComplete() {
    ...
    } catch (err) { setCanRetry(true); setApplyError(err instanceof Error ? err.message : "Failed to apply"); return; }
  ...
  function clearAll() { setSearchQ(""); setActiveFilter("all"); }
        ...
        <div className="tpl-applied-banner">
          ...
          <div className="tab-body">
            ...
            {!appliedInView && isFiltered && (
              <div className="tab-filtered-warn">↑ Currently filtered out — clear filters to see it</div>
            )}
          </div>
          <button
      ...
      {applyError && (
        <div className="tpl-error-banner">
          ...
          <div className="tpl-error-actions">
            {canRetry && (
              <button className="tpl-error-retry" onClick={handleRetry}>Retry</button>
            )}
      ...
      <div className="tpl-cards-scroll" ref={scrollRef}>
        {filtered.length === 0 ? (
          <div className="tpl-empty">
            ...
            <button className="tpl-empty-clear" onClick={clearAll}>Clear filters →</button>
          </div>
        ) : (
            ...
            {filtered.map((tpl) => {
              ...
              return (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={`${tpl.name}${isLocked ? " (Pro template — upgrade to use)" : ""}`}
